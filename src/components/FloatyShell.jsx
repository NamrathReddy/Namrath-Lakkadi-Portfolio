        ...
        <div className="h-full flex flex-col gap-6 p-6 justify-center items-center">
          {sectionConfig.map((cfg,index) => (
            <button
              ...
              className= {` ${index === mainIndex?"text-black bg-slate-800":`text-black xl:hover:bg-white ${cfg.bgClass}`} font-semibold px-3 py-2 shadow-sm  rounded-full w-full md:h-[5dvh] md:w-1/2 `}
            >
              {cfg.label}
            </button>
          ))}
